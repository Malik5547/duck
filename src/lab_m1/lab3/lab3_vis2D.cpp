...
{
    const int spaceMoveSpeed = 1;
    const int spaceScaleSpeed = 1;

    // TODO(student): Move the logic window with W, A, S, D (up, left, down, right)
    if (window->KeyHold(GLFW_KEY_W)) {
        logicSpace.y += spaceMoveSpeed * deltaTime;
    }
    if (window->KeyHold(GLFW_KEY_D)) {
        logicSpace.x += spaceMoveSpeed * deltaTime;
    }
    ...
    if (window->KeyHold(GLFW_KEY_Z)) {
        logicSpace.height -= spaceScaleSpeed * deltaTime;
        logicSpace.width -= spaceScaleSpeed * deltaTime;
    }
